...
public class ex3 {
  ...
  static int[] myAlgorithm (double[] taskTimes, int numProcessors)
  {
    ...
    int maxl = Math.round(taskTimes.length / numProcessors);
    int temp = 0, max = 0, i, j, k;

    int[] tmpPartition = new int[maxl];
    int[] tmpPartition2 = new int[maxl];
    ...
    for (i = 0; i < taskTimes.length; i++) {
      ...
      if (temp > max) {
        ...
        for (j = 0; j < maxl; j++) {
          tmpPartition2[j] = tmpPartition[j];
        }
    ...
    for (i = 0; i < numProcessors; i++) {
      partition[i] = tmpPartition2[tmpPartition2.length];
    }
